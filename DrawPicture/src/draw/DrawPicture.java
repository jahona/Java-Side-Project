...
implements MouseListener,MouseMotionListener,ItemListener, ActionListener {
	...
	public void paint(Graphics g) {
		...
		if(dist == 1 || dist == 0){
			...
			if(fill){
				g.fillRect(tempx, tempy, Math.abs(x1-x), Math.abs(y1-y));
			}else{
				g.drawRect(tempx, tempy, Math.abs(x1-x), Math.abs(y1-y));
			}
		}else if(dist == 3){
			if(fill){
				g.fillOval(tempx, tempy, Math.abs(x1-x), Math.abs(y1-y));
			}else{
				g.drawOval(tempx, tempy, Math.abs(x1-x), Math.abs(y1-y));
			}
		}
		...
		for(int i = 0; i<vc.size(); i++){
			...
			if(imsi.getType() == 1|| imsi.getType() == 0){
				...
				if(imsi.isFill()){
					g.fillRect
					(imsi.getX(), imsi.getY(), Math.abs(imsi.getX1() - imsi.getX()), Math.abs(imsi.getY1() - imsi.getY()));
	...
	public void mouseReleased(MouseEvent e) {
		...
		DrawInfo di = new DrawInfo(x<x1 ? x : x1, y<y1 ? y : y1,
				x<x1 ? x1 : x, y<y1 ? y1 : y, dist, c, opfill.getState());
